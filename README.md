...
The browser app lives in `web/` and is the fastest way to test the product from a PC, iPad browser, or iPhone browser before TestFlight is ready. The current web beta app version is `0.2.8`, sourced from `web/package.json` and visible in Settings > Beta.
...
When filing beta notes, include app version `0.2.8`, browser/device, workspace role, and whether the pass used demo mode or authenticated Supabase mode.
